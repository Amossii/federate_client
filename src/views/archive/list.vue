<template>
  <el-main class="ea-table">
    ...
    <el-table :data="table.data">
      <el-table-column label="文件名称" prop="filename" fixed width="160" sortable="custom" />
  ...
  <el-dialog v-model="dialog.visible" :title="dialog.title" :close-on-click-modal="false" destroy-on-close
    ...
    <el-form ref="dialogRef" :model="dialog.form" :rules="dialog.formRule" label-position="left" label-width="80px">
      <el-form-item label="状态" prop="status">
        <el-switch v-model="dialog.form.status" :active-value="1" active-text="上架" :inactive-value="0"
          inactive-text="下架" />
      </el-form-item>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-select v-if="dialog.formTempSelectType || dialog.form.archiveCategoryId"
              v-model="dialog.form.archiveCategoryId" placeholder="选二级分类">
              <template v-for="item in dialog.vodTypeList">
                <el-option v-if="dialog.form.archiveCategoryId
                  || dialog.formTempSelectType == item.parent
                  || dialog.formTempSelectType == item.id" :key="item.id" :label="item.name" :value="item.id" />
              </template>
            </el-select>
          </el-col>
        </el-row>
      ...
      <el-form-item v-if="dialog.formAction == 'edit'" label="标签栏" prop="status">
        <el-tag v-for="tag in dialog.form.Tags" :key="tag.id" closable style="margin: 6px;"
        ...
        <el-select v-model="dialog.formTempSelectTag" filterable remote reserve-keyword placeholder="搜索标签名称"
          :remote-method="handleRemoteTagList" size="small" @change="handleRemoteTagSelect">
