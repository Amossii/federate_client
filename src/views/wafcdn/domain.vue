<template>
  ...
  <el-main class="ea-table">
    ...
    <el-table :data="table.data">
      ...
      <el-table-column label="端口(http)" prop="port" align="center" />
      <el-table-column label="端口(https)" prop="ssl.port" align="center" />
      <el-table-column label="源站" align="center" width="300">
      ...
      <el-table-column label="缓存目录" prop="cache.dir" align="center" />
      <el-table-column fixed="right" label="操作" width="105">
        <template #default="scope">
          <el-button link type="primary" @click="handleDialogEdit(scope.row)">
            编辑
          </el-button>
          <el-button link type="danger" @click="handleDialogDelete(scope.$index)">
            删除
          </el-button>
        </template>
      </el-table-column>
  ...
  <el-dialog v-model="dialog.visible" :title="dialog.title" :close-on-click-modal="false" destroy-on-close
    ...
    <el-form ref="dialogRef" label-position="left" :model="dialog.formTmp" :rules="dialog.formRule">
      <el-tabs type="border-card">
        ...
        <el-tab-pane label="回源管理">
          <el-form-item label="回源URL" prop="backend.url">
            <el-input v-model="dialog.formTmp.backend.url" />
          </el-form-item>
          <el-form-item label="回源Host" prop="backend.host">
            <el-input v-model="dialog.formTmp.backend.host" />
          </el-form-item>
          <el-form-item label="回源请求头">
            <template v-for="(value, index) in dialog.formTmp.backend.req_header" :key="index">
              <el-col :span="11">
                <el-input v-model="value.name" placeholder="名" />
              </el-col>
              <el-col :span="2" style="text-align: center;">
                <el-button icon="Minus" circle @click="dialog.formTmp.backend.req_header.splice(index, 1)" />
              </el-col>
              <el-col :span="11">
          ...
          <el-form-item label="回源返回头">
            <template v-for="(value, index) in dialog.formTmp.backend.resp_header" :key="index">
              <el-col :span="11">
                <el-input v-model="value.name" placeholder="名" />
              </el-col>
              <el-col :span="2" style="text-align: center;">
                <el-button icon="Minus" circle @click="dialog.formTmp.backend.resp_header.splice(index, 1)" />
              </el-col>
              <el-col :span="11">
                <el-input v-model="value.value" placeholder="值" />
              </el-col>
        ...
        <el-tab-pane label="SSL">
          <el-form-item label="SSL端口" prop="ssl.port">
            <el-input-number v-model="dialog.formTmp.ssl.port" />
          </el-form-item>
          <el-row>
            <el-col :span="11">
              <el-form-item label="证书" prop="ssl.crt">
                <el-input v-model="dialog.formTmp.ssl.crt" rows="15" type="textarea" placeholder="crt证书文件" />
              </el-form-item>
            </el-col>
            <el-col :span="11">
              <el-form-item label="密钥" prop="ssl">
                <el-input v-model="dialog.formTmp.ssl.key" placeholder="key私钥文件" rows="15" type="textarea" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-tab-pane>
        <el-tab-pane label="缓存配置">
          <el-form-item label="缓存路径" prop="cache.dir">
            <el-input v-model="dialog.formTmp.cache.dir" />
          </el-form-item>
          <el-form-item label="缓存规则">
            <template v-for="(value, index) in dialog.formTmp.cache.rule" :key="index">
              <el-col :span="11">
                <el-input v-model="value.path" placeholder="名" />
              </el-col>
              <el-col :span="2" style="text-align: center;">
                <el-button icon="Minus" circle @click="dialog.formTmp.cache.rule.splice(index, 1)" />
              </el-col>
              <el-col :span="11">
                <el-input-number v-model="value.time" placeholder="值(分钟)" />
              </el-col>
              <el-col :span="24" style="height: 5px;" />
            </template>
            <el-button type="primary" icon="Plus" circle
